...
0:00锚点固定为0，在10分钟区间中点取值。正式场景采用`paired_residual_weighted`：
目标日负荷中心来自无泄漏的星期/近期预测，前31天中同一历史日的负荷伪预测残差与
对应发布时间PV预报残差保持配对，再按`tau=14`时间衰减和同星期2倍加权。
2月1日因1月1日缺少更早负荷历史使用30条场景，其余日期31条；`raw_equal`仍可精确复现旧逻辑。

升级模型重新回测后，官方主策略仍为 `Q3_FINAL_UPDATE_HOURS=(0,6,12)`，与第四问保持一致。
每次合同优化只覆盖剩余时域，SOC从已执行段末继承，终端回到6000 kWh；合同仅在
选中的预报时刻更新，储能则在全部144个slot逐点观察实际净负荷、求解MPC并只执行首步。
...
全部策略均使用相同334天、相同历史窗口和结算参数。完整运行会重新比较六种策略并选择
总费用最低者；当前确定性回测仍选中三时刻策略。四时刻策略保留为消融对照；
`ISSUE_HOURS=(0,6,12,18)`仍表示附件3全部预报发布时间，用于输入校验和预测精度评价。
比较包含逐步增加更新和逐一删除更新，保留负的费用节省值。
除合同LP外，每种策略每天执行144次小型因果MPC，无商业求解器依赖。

升级后的`0+6+12`总费用为14,641,650.05元，比旧`raw_equal + frozen`降低
2,378,656.59元（13.9754%）。固定更新时间的四组模型消融表明：仅配对残差场景节省
2,096,657.36元，仅因果储能节省310,935.97元；组合后紧急购电量和紧急购电费分别为
161,192.61 kWh和560,743.27元。六组更新时间中`0+6+12`比不调整合同再节省262,907.55元，
而加入18:00更新反而多支出1,223.81元，因此不使用18:00预报调整合同。
...
| `target/question3/model_ablation_summary.csv` / `.json` | 固定0+6+12下场景模型与储能执行的四组消融 |
...
4-2复用问题2选定的`residual_weighted_causal`（A7），4-3复用问题3的
`paired_residual_weighted + causal_mpc`；历史残差、光伏预测插值、合同优化、逐slot储能
和滚动SOC均调用已有模块，优化器未复制。
...
4-2动态价总费用为15,265,670.46元，比同一A7策略的固定价费用增加576,387.10元
（3.9239%）；4-3动态价总费用为15,192,251.26元，比同一问题3策略的固定价费用增加
550,601.21元（3.7605%）。合同滚动修正使4-3比4-2再降低73,419.19元。
相对升级前的动态价实现，4-2和4-3分别降低2,657,867.64元（14.8289%）和
2,052,303.16元（11.9012%）。动态价下4-2/4-3应急购电量分别为240,877.06和
157,938.87 kWh；完整价格曲线提高了费用，但4-3的日内修正仍有效降低应急风险。
...
重绘时核对价格快照、输入SHA-256、schema 2策略标识和全部物理/费用约束；输入变化时须重新运行。
