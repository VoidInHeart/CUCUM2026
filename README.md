...
- A7只用目标日前数据分别预测负荷/PV，并搬移同一历史日的配对预测误差；场景概率采用
  `tau=14` 的时间衰减与同星期2倍权重。334天总费用为14,689,283.36元，相对A0节省
  2,920,508.33元（16.5846%），计划费和紧急购电费分别减少1,907,160.63元和1,013,347.70元。
- level-scale对照使紧急购电费增加547,795.08元，因此不作为正式策略。
