...
def solve_storage_mpc(
    ...
    if not result.success:
        raise RuntimeError(
            f"Question2 storage MPC failed on {context}: HiGHS status {result.status}: {result.message}"
        )
